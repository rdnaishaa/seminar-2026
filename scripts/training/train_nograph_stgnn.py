...
# No-Graph Control:
# hanya self-connection, tidak ada informasi dari corridor lain
A = np.eye(
    len(corridors),
    dtype=np.float32
)
...
class FixedGraphConv(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # Informasi dari graph
        # Untuk No-Graph, adjacency = Identity
        graph_x = tf.einsum(
            "ij,btjf->btif",
            self.adjacency,
            inputs
        ...
        # Pertahankan raw node information
        x = tf.concat(
            [inputs, graph_x],
            axis=-1
        )
...
pd.DataFrame(
    ...
    RESULT_DIR /
    "nograph_stgnn_training_history.csv",
    index=False
)
...
model.save_weights(
    RESULT_DIR /
    "nograph_stgnn.weights.h5"
)


print("\n=== SAVED ===")
print("outputs/results/nograph_stgnn_results.csv")
print("outputs/results/nograph_stgnn_predictions.csv")
print("outputs/results/nograph_stgnn_training_history.csv")
print("outputs/results/nograph_stgnn.weights.h5")
